		...
		*/
		...
		TaskContainer m1=new TaskContainer(new Workstation("Ebatlama")),m2=new TaskContainer(new Workstation("Bantlama")),
				m3=new TaskContainer(new Workstation("Montaj\t")), m4=new TaskContainer(new Workstation("Paketleme")),
						m5=new TaskContainer(new Workstation("Sevkiyat"));
		m3.getElement().bufferCapacity=1;
		//List<SimulationEvent> events=new ArrayList<>();
		
		Entity p1=new Entity(),p2=new Entity(),p3=new Entity(),p4=new Entity(),p5=new Entity();
		
		AssemblyContainer[] inputs=new AssemblyContainer[] {new AssemblyContainer(p1,1), new AssemblyContainer(p2,1)};  
		AssemblyContainer[] outputs= {new AssemblyContainer(p3,1,1)}; //, new TransferableElementContainer(p4,1,1)
		
		AssemblyContainer[] inputs2=new AssemblyContainer[] {new AssemblyContainer(p3,1), new AssemblyContainer(p4,1)}; 
		AssemblyContainer[] outputs2= {new AssemblyContainer(p5,1,1)}; //, new TransferableElementContainer(p4,1,1)
		...
		p1.addTarget(new TransferInfo(new Process(new TaskNode(m1,new FixedTimeParameter(40, TimeUnit.second))))); //output, new TaskNode(m3,"30", inputs)
		...
		p3.addTarget(new TransferInfo(new Process(inputs2, outputs2, new TaskNode(m5,new FixedTimeParameter(20, TimeUnit.second)))));
		p4.addTarget(new TransferInfo(new Process(new TaskNode(m3,new FixedTimeParameter(20, TimeUnit.second)))));
		p4.addTarget(new TransferInfo(new Process(new TaskNode(m4,new FixedTimeParameter(15, TimeUnit.second)))));
		p4.addTarget(new TransferInfo(new Process(inputs2, outputs2, new TaskNode(m5,new FixedTimeParameter(20, TimeUnit.second)))));
